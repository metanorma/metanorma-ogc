...
RSpec.describe IsoDoc::Ogc do
  it "processes terms and definitions" do
    ...
    INPUT
    expect(xmlpp(File.read("test.doc").gsub(%r{^.*<div class="WordSection3">}m, "<body><div class='WordSection3'>").gsub(
                   %r{</body>.*}m, "</body>"
  ...
  it "populates Word ToC" do
    ...
    INPUT
    presxml = IsoDoc::Ogc::PresentationXMLConvert.new({}).convert("test",
                                                                  input, true)
    ...
    expect(xmlpp(word.gsub(/_Toc\d\d+/,
                           "_Toc"))).to be_equivalent_to xmlpp(<<~'OUTPUT')
                             <div>
  ...
  it "processes boilerplate" do
    ...
    IsoDoc::Ogc::WordConvert
      ...
      INPUT
    expect((File.read("test.doc").gsub(%r{^.*<div class="boilerplate-copyright">}m, "<div class='boilerplate-copyright'>").gsub(
      %r{<div class="warning">.*}m, ""
    ))).to be_equivalent_to xmlpp(<<~"OUTPUT")
    ...
    OUTPUT
    expect((File.read("test.doc").gsub(%r{^.*<div class="boilerplate-license">}m, "<div class='boilerplate-license'>").gsub(
      %r{<p class="license">.*}m, '<p class="license"/></div></div>'
    ))).to be_equivalent_to xmlpp(<<~"OUTPUT")
